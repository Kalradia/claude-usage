...
Client-side UI state (collapsed sections, the 24h update-check cache) is kept in **`localStorage`**, which is keyed by the page's origin — stable across reloads as long as host/port don't change.
