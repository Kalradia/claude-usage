...
#
#   Brew reads the formula from the tap's default branch (main), never from develop
#   or a tag. Run this on develop while prepping a release; the change reaches brew
#   users through the normal develop -> main release merge. It never touches main
...
#           previous release. Review the diff, then commit on develop.
#
...
echo
echo "➡   Review, then commit on develop. It ships to brew users at the next develop -> main release merge."
